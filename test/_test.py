...
EXISTING_CROJECT = Path(DOCANALYSIS_TOP, 'stem_cell_research_300')
DICT_DIRECTORY = Path(DOCANALYSIS_TOP, 'ethics_dictionary')
TEST_DICT = Path(DICT_DIRECTORY, 'ethics_demo', 'ethics_demo.xml')
TEMP_CPROJECT = Path(DOCANALYSIS_TOP, 'test_ethics_30')
HITS = 20

def test_CProject_exists(self):
    """asserts whether CProject directory exists
    """
    assert EXISTING_CROJECT.exists(), f"checking whether {EXISTING_CROJECT} exists"
...
def test_dict_creation_entites(self):
    os.system(f'docanalysis --project_name {TEMP_CPROJECT} --dictionary {TEST_DICT} --entity_extraction --key_phrase_extraction --make_dictionary keyphrase')
